...
def get_room_info(html):  # 获取信息
    doc = pq(html)
    room_info = doc('div').filter('.content__list > div').items()
    list_name = []
    for i in room_info:
        list_name.append(i.attr('title'))


#%%
my_html = get_one_page_html('https://sh.lianjia.com/zufang/rs%E5%BE%A1%E6%A1%A5/')
...
t = get_one_page_html('https://sh.lianjia.com/zufang/rs%E5%BE%A1%E6%A1%A5/')

#%%

doc = pq(t)
a = doc('div.wrapper > div >div.content.w1150 > div.content__article > div.content__list')
b = doc('div').filter('.content__list > div')
#%%
def get_room_url(html):  # 获取当前页面上所有room_info的url
    doc = pq(html)
    room_urls = doc('div').filter('.content__list > div').items()
    return room_urls
